...
function BaseN(options) {
    ...
    if (opt.base && opt.characters) {
        throw new Error('cannot specify both base and characters');
    }
 ...
 * @returns {Number}
 */
BaseN.prototype._findLength = function(max) {
 ...
 */
BaseN.prototype._pad = function(id) {
...
BaseN.prototype.encode = function(num) {
    ...
    assert.equal(num >= 0, true, 'number must be positive');
    ...
    if (self.length !== null) {
        ...
        if (out.length > self.length) {
            throw new Error('base10 value of ' + num + ' (encoded: ' + out +
 ...
 */
BaseN.prototype.decode = function(id) {
    ...
    while (i--) {
        var character = id[i];
        var charIdx = self.characters.indexOf(character);
        var numToAdd;
        ...
        if (numToAdd === -1) {
            throw new Error('unknown ' + character + ' character encountered');
        } else {
            num += numToAdd;
        }
